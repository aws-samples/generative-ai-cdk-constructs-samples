...
LAMBDA_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "lambda", "contract_import", "extract_clause_types_fn")


class ExtractClauseTypesStep(Construct):
    """CDK construct for clause types extraction step"""
    ...
    def __init__(
        ...
        self.extract_clause_types_fn = stack_constructs.PythonFunctionConstruct(
            self,
            "ExtractClauseTypesFunction",
            entry=LAMBDA_PATH,
            index="index.py",
            handler="handler",
            runtime=lambda_.Runtime.PYTHON_3_12,
            architecture=lambda_.Architecture.X86_64,
            timeout=Duration.minutes(15),  # Longer timeout for clause analysis
        ...
        self.extract_clause_types_fn.role.add_to_policy(iam.PolicyStatement(
            actions=[
                "ssm:GetParameter",
            ],
            resources=[
        ...
        stack_constructs.add_bedrock_marketplace_permissions(self.extract_clause_types_fn.role)

        self.sfn_task = tasks.LambdaInvoke(
            self,
            "Extract Clause Types",
            lambda_function=self.extract_clause_types_fn,
        ...
        contract_bucket.grant_read(self.extract_clause_types_fn)
        import_jobs_table.grant_read_write_data(self.extract_clause_types_fn)

        # CDK Nag suppressions
        NagSuppressions.add_resource_suppressions(
            construct=self.extract_clause_types_fn.role,
            suppressions=[
                NagPackSuppression(
                ...
                NagPackSuppression(
                    ...
                    reason="Bedrock model access for extracting clause types from reference contracts. Model ID is user-configurable via SSM parameter.",
                ),
                NagPackSuppression(
                    id="AwsSolutions-IAM5",
