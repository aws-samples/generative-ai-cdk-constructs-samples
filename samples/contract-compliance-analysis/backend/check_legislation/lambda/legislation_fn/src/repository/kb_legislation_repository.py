#
# Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License"). You may not use this file except in compliance
# with the License. A copy of the License is located at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# or in the 'license' file accompanying this file. This file is distributed on an 'AS IS' BASIS, WITHOUT WARRANTIES
# OR CONDITIONS OF ANY KIND, express or implied. See the License for the specific language governing permissions
# and limitations under the License.
#

from typing import List

from model import Legislation
from repository import LegislationRepository
from repository.opensearch_client import OpenSearchLegislationClient
from repository.bedrock_agent_client import BedrockAgentLegislationClient


class KBLegislationRepository(LegislationRepository):
    """
    Repository that coordinates between OpenSearch and Bedrock Agent clients
    for legislation management operations.
    """

    def __init__(
        self,
        opensearch_client: OpenSearchLegislationClient,
        bedrock_agent_client: BedrockAgentLegislationClient,
    ):
        self.opensearch_client = opensearch_client
        self.bedrock_agent_client = bedrock_agent_client

    def list_legislations(self) -> List[Legislation]:
        """List all legislations using OpenSearch."""
        return self.opensearch_client.list_legislations()

    def ingest_legislation(self, legislation: Legislation) -> str:
        """Ingest legislation using Bedrock Agent."""
        return self.bedrock_agent_client.ingest_legislation(legislation)
