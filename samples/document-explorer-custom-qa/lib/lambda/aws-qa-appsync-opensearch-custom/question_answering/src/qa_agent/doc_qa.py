...
def run_qa_agent_rag_no_memory(input_params):
    ...
    try:
        ...
        logger.info(f'source count :: {len(source_documents)}')
    ...
    # split filename into separate filenames
    filenames = input_params['filename'].split(', ')
    filtered_source_documents = []
    for output_file_name in filenames:
        logger.info(f'output_file_name is :: {output_file_name}')
        if output_file_name:
            file_without_extension = os.path.splitext(output_file_name)[0]
            filtered_source_documents += [doc for doc in source_documents if file_without_extension == os.path.splitext(doc.metadata['source'])[0]]
    
    source_documents = filtered_source_documents
            
    logger.info(f'filtered source is :: {source_documents}')
    logger.info(f'filtered source count :: {len(source_documents)}')
    status_variables['sources'] = list(set(doc.metadata['source'] for doc in source_documents))
    logger.info(f'sources are :: {status_variables["sources"]}')
