...
def receive_messages():
    ...
    if 'Messages' in response:
        ...
        for message in messages:
            ...
            reformulated_question = message_body.get('reformualted_question', {}).get('value', '')
            st.session_state["reformulated_question"] = reformulated_question
            
            print(f'set reformulated_question :: {reformulated_question} ')
            original_user_question = message_body.get('user_question', {}).get('value', '')
            ...
            if generated_query:
                with st.chat_message("assistant"):
                     assitatnt_response =" The Generated query is :: "+generated_query+" If you  fine with the generated question , Please type yes, if not then please enter the new query starting with "+generated_query_key
                ...
                #st.session_state["poll_feedback_queue"] =  False
                 ...
                 with st.chat_message("assistant"):
                     assitatnt_response =" Reformulated question is :: "+reformulated_question+" The question has been reformulated as per the DB tables. If you are fine with the reformulated question , Please type yes, if not then please enter the question starting with "+reformulated_ques_key
                     st.markdown(assitatnt_response)
            else:
                with st.chat_message("assistant"):
                     st.markdown("No reformulated question found in the message. proceding with query generation..")
            
            print(" Message displayed...")
            st.session_state.messages.append({"role": "assistant", "content": assitatnt_response})
...
def receive_result():
    ...
    if 'Messages' in response:
        ...
        for message in messages:
            ...
            # st.session_state["poll_feedback_queue"] =  False
            print(" requesting to stop polling for result")
    else:
...
def send_text_to_api(input_text,unique_id):
    ...
    response = requests.post(TEXT_TO_SQL_API_ENDPOINT,headers=headers, json=payload)
...
if auth.is_authenticated():
    ...
    if prompt := st.chat_input("Please ask your question"):
        ...
        reformulate_key_match = re.search(rf'{reformulated_ques_key}\s*(.*)', prompt)
        generate_query_key_match = re.search(rf'{generated_query_key}\s*(.*)', prompt)
        ...
        if reformulate_key_match or prompt.lower() == "yes" :
            ...
            if(prompt.lower()  == "yes"):
                ...
                overwrite_reformulated_question = reformulate_key_match.group(1).strip()    
         
            send_feedback(reformulated_ques_key,overwrite_reformulated_question, unique_id)
            ...
            if(prompt.lower()  == "yes"):
                ...
                overwrite_generated_query = generate_query_key_match.group(1).strip() 
                            
            send_feedback(generated_query_key,overwrite_generated_query,unique_id)
        else:
...
with st.sidebar:
    ...
    model_id = st.selectbox(
        ...
        key="model_id",
        help="Select model type to generate query.",
    )
