...
namespace ChatbotDemo.Api.Handlers;
...
public abstract class SqsEventHandler<TMessage> where TMessage : class, new()
{
    ...
    /// <returns></returns>
    public abstract Task ProcessSqsMessage(SQSEvent.SQSMessage message, ILambdaContext lambdaContext);
    ...
    [Tracing(Namespace = "SqsEventHandler", SegmentName = "SqsEventHandler", CaptureMode = TracingCaptureMode.Error)]
    ...
    private async Task ProcessEvent(SQSEvent sqsEvent, ILambdaContext lambdaContext)
    {
        ...
        foreach (var sqsMessage in sqsMessages)
        {
            try
            {
                Message = JsonSerializer.Deserialize<TMessage>(sqsMessage.Body);

                // This abstract method is implemented by the concrete classes i.e. ProcessEmployeeFunction.
                await ProcessSqsMessage(sqsMessage, lambdaContext);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex);
