...
def generate_summary(source_filename):
    ...
    if auth.is_authenticated():
        ...
        # Get summary job ID 
        summary_job_id = decoded_token.get("cognito:username", id_token[:10])+"-"+source_filename
...
def on_message_update(message, subscription_client):
    ...
    if status == "New LLM token":
        ...
        summary_widget.text_area( '',st.session_state.summary_widget_text + " ▌",height=300)
...
def subscribe_to_summary_updates():
    ...
    if auth.is_authenticated():
        ...
        summary_job_id = decoded_token.get("cognito:username", id_token[:10])+"-"+st.session_state['uploaded_filename']
...
if auth.is_authenticated():
    ...
    with st.form("file-uploader", clear_on_submit=True):
        ...
        uploaded_files = st.file_uploader('Upload an image', type=['jpeg','png','jpg'],accept_multiple_files=True)
                ...
                with col2:
                    ...
                    with st.spinner('Processing...'):
                        if (st.session_state['file_processed']==st.session_state['uploaded_filename']):
                            ...
                            summary_widget.text_area( '',summary_text + " ▌",height=300)
